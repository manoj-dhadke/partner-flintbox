...
connector_name_response = @call.connector("ssh")
                               ...
                               .set("command","service cron start")    #Command to be executed on target machine
...
if connector_name_output == ""
   @output.set("result","Service is stopped, need to start")
   @log.info("#{connector_name_output}")
end

if connector_name_output != ""
   @output.set("result","Service is running, need to restart")
   @log.info("#{connector_name_output}")
end
