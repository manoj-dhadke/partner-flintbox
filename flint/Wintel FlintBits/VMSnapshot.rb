...
@vm = @input.get("vmname")
@snapshot=@input.get("snapshotname")
@quote = "'"
# Creating a checkpoint or snapshot of VM, getting the VM name & snapshot name from User.
@command = "Get-VM -Name " instance_variable_get("@vm") | "Checkpoint-VM -SnapshotName" + instance_variable_get("@snapshot")
